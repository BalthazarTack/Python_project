...
def test_hex_iteration():
    ...
    assert result.shape == (expected_num_rows, expected_num_cols), (
        f"Expected result shape to be {(expected_num_rows, expected_num_cols)} but got"
        f" {result.shape}"
    )
    ...
    assert result[0][1] == (
        1.5 * hex_size,
        expected_y_offset,
    ), f"Expected ({1.5 * hex_size}, {expected_y_offset}), but got {result[0][1]}"

    assert result[1][0] == (
        0,
        np.sqrt(3) * hex_size,
    ), f"Expected (0, {np.sqrt(3) * hex_size}), but got {result[1][0]}"
...
def test_flat_hex_corner():
    ...
    for i in range(6):
        assert np.allclose(
            flat_hex_corner(center, size, i)[0], expected_points[i][0]
        ), (
            f"Expected {expected_points[i][0]} , but got"
            f" {flat_hex_corner(center, size, i)[0]}"
        )
        assert np.allclose(
            flat_hex_corner(center, size, i)[1], expected_points[i][1]
        ), (
            f"Expected {expected_points[i][1]} , but got"
            f" {flat_hex_corner(center, size, i)[1]}"
        )
